...
VERIFICATION_ROLE_NAME = os.getenv("VERIFICATION_ROLE_NAME", "Member")
...
# ---------------------------------------------------------------------------
VERIFICATION_FILE = os.path.join(os.path.dirname(__file__), "verifications.json")
...
def is_verified(user_id: int, guild_id: int) -> bool:
    data = load_verifications()
    return str(user_id) in data.get(str(guild_id), {})

def mark_verified(user_id: int, guild_id: int):
    data = load_verifications()
    gdata = data.get(str(guild_id), {})
    gdata[str(user_id)] = {"verified_at": time.time()}
    data[str(guild_id)] = gdata
    save_verifications(data)
...
async def profile_cmd(interaction: discord.Interaction, member: discord.Member | None = None):
    ...
    # Get verification
    verified = is_verified(target.id, guild.id)
...
class FAQView(discord.ui.View):
    ...
    @discord.ui.button(label="Search FAQ", emoji="🔍", style=discord.ButtonStyle.primary, custom_id="faq_search")
...
# ---------------------------------------------------------------------------
# Verification system: welcome DM + reaction verify
# ---------------------------------------------------------------------------
    ...
    try:
        ...
        e = discord.Embed(title=f"Welcome to {guild.name}!",
                          description=(
                              ...
                              "To get access to the server, please verify in "
                              f"{verify_ch.mention if verify_ch else '#verify'}: "
                              "react with ✅ to the verification message.\n\n"
                              "Once verified, you'll get the "
                              f"{role.mention if role else 'Member'} role and full access."
                          ),
...
# Verification panel in #verify channel
class VerifyView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)

    @discord.ui.button(label="Verify Me", emoji="✅", style=discord.ButtonStyle.success, custom_id="verify_btn")
    async def verify(self, interaction: discord.Interaction, button: discord.ui.Button):
        guild = interaction.guild
        role = discord.utils.get(guild.roles, name=VERIFICATION_ROLE_NAME)
        if role is None:
            await interaction.response.send_message("Verification role not set up. Contact staff.", ephemeral=True)
            return
        ...
        try:
            ...
            mark_verified(interaction.user.id, guild.id)
...
async def setup_verification_cmd(interaction: discord.Interaction):
    ...
    e = discord.Embed(title="🔐 Server Verification",
                      description=(
                          "Click **Verify Me** below to get the Member role and full server access.\n\n"
                          "This helps prevent spam and ensures everyone agrees to the rules."
                      ),
