...
async def invite_cmd(interaction: discord.Interaction):
    """Generates OAuth2 invite link with Administrator permission."""
    ...
    await interaction.response.send_message(
        f"Invite link (Administrator + commands):\n{url}",
        ephemeral=True
    )


# ---------------------------------------------------------------------------
# /toggle - Framework for bot modes (security, games, etc.)
# ---------------------------------------------------------------------------
TOGGLE_FILE = os.path.join(os.path.dirname(__file__), "toggles.json")
...
# Define available modes (not implemented yet - just placeholders)
AVAILABLE_MODES = {
    "security": "Anti-spam, link filtering, mention protection, raid protection",
    "games": "Economy, minigames, leaderboards, daily rewards",
...
@app_commands.checks.has_permissions(manage_guild=True)
@bot.tree.command(name="toggle", description="Enable/disable bot modes (admin).")
@app_commands.describe(mode="Mode to toggle", enabled="Enable or disable the mode")
@app_commands.choices(mode=[
    app_commands.Choice(name=k, value=k) for k in AVAILABLE_MODES.keys()
])
async def toggle_cmd(interaction: discord.Interaction, mode: str, enabled: bool):
    """Toggle bot modes on/off. Modes are not implemented yet - framework only."""
    if mode not in AVAILABLE_MODES:
        await interaction.response.send_message(f"Unknown mode. Available: {', '.join(AVAILABLE_MODES.keys())}", ephemeral=True)
        return
    ...
    data[guild_id][mode] = enabled
    save_toggles(data)
    
    status = "enabled" if enabled else "disabled"
    await interaction.response.send_message(
        f"Mode **{mode}** {status}.\n"
        f"*Note: Modes are not implemented yet - this is just the framework.*",
        ephemeral=True
    )


@bot.tree.command(name="toggles", description="View current mode status.")
async def toggles_cmd(interaction: discord.Interaction):
    data = load_toggles()
    guild_id = str(interaction.guild_id)
    guild_toggles = data.get(guild_id, {})
    
    e = discord.Embed(title="Bot Modes Status", color=discord.Color.blurple())
    for mode, desc in AVAILABLE_MODES.items():
        status = "🟢 Enabled" if guild_toggles.get(mode) else "🔴 Disabled"
        e.add_field(name=f"{status} — {mode}", value=desc, inline=False)
    
    if not guild_toggles:
