...
# ---------------------------------------------------------------------------
# /dash - Admin dashboard in Discord (ephemeral, interactive)
# ---------------------------------------------------------------------------
...
def build_dash_embed(guild: discord.Guild) -> discord.Embed:
    ...
    staff = staff_role(guild)
    senior = senior_role(guild)
    e = discord.Embed(title="Bot Dashboard", color=discord.Color.blurple(),
                      timestamp=datetime.datetime.now(datetime.timezone.utc))
    ...
    e.add_field(name="Staff roles", value=f"{staff.mention if staff else '❌'} / {senior.mention if senior else '❌'}", inline=True)
    if opens:
        e.add_field(name="Oldest open", value=", ".join(c.mention for c in opens[:5]), inline=False)
    e.set_footer(text="Admin-only. Buttons expire after 5 min — re-run /dash.")
    return e
...
class DashSettingsModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        env_path.write_text("\n".join(new_lines) + "\n")
        await interaction.response.send_message(
            "Settings saved to `.env`. **Run `/restart` to apply.**", ephemeral=True)
